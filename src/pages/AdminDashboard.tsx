...
const AdminDashboard = () => {
  ...
  const fetchMaterials = async (page = 1, limit = materialsPerPage) => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('content').select('*').order('created_at', {
        ascending: false
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const softwareCompatibilityArray = formData.software_compatibility ? formData.software_compatibility.split(',').map(s => s.trim()).filter(s => {
      ...
      let result;
      if (editingMaterial) {
        result = await supabase.from('content').update(materialData).eq('id', editingMaterial.id).select();
      } else {
        result = await supabase.from('content').insert([materialData]).select();
      }
      if (result.error) {
  ...
  return <div className="min-h-screen bg-background p-2 sm:p-4 lg:p-6">
      <div className="max-w-full sm:max-w-6xl mx-auto">
                ...
                <Tabs defaultValue="basic" className="w-full">
                  ...
                  <TabsContent value="basic" className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input id="file_url" value={formData.file_url} onChange={e => setFormData({
                      ...formData,
                    ...
                    <div>
                      ...
                      <div className="mt-2 space-y-4">
                        <div className="flex items-center gap-4">
                          <Button type="button" variant="outline" disabled={uploadingThumbnail} onClick={() => document.getElementById('thumbnail-upload')?.click()} className="flex items-center gap-2">
                            {uploadingThumbnail ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-red-500"></div> : <Image className="w-4 h-4" />}
                            {uploadingThumbnail ? 'Uploading...' : 'Upload Thumbnail'}
                          </Button>
                          <input id="thumbnail-upload" type="file" accept="image/*" onChange={handleThumbnailUpload} className="hidden" />
                        </div>
                        
                        {formData.thumbnail_url && <div className="flex items-center gap-4">
                            <img src={formData.thumbnail_url} alt="Thumbnail preview" className="w-20 h-20 object-cover rounded-lg border" />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      ...formData,
                      tags: e.target.value
                    })} placeholder="e.g., cinematic, moody, vintage" className="mt-1" />
                      </div>

                      <div>
                        <Label htmlFor="software_compatibility" className="text-sm font-medium">Software Compatibility</Label>
                        <Input id="software_compatibility" value={formData.software_compatibility} onChange={e => setFormData({
                      ...formData,
                      software_compatibility: e.target.value
                    })} placeholder="e.g., premiere_pro, after_effects" className="mt-1" />
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                      <div className="flex items-center space-x-2">
                        <Switch id="is_premium" checked={formData.is_premium} onCheckedChange={checked => setFormData({
                      ...formData,
                      is_premium: checked
                    })} />
                      ...
                      <div className="flex items-center space-x-2">
                        <Switch id="is_featured" checked={formData.is_featured} onCheckedChange={checked => setFormData({
                      ...formData,
                      is_featured: checked
                    })} />
                  ...
                  <TabsContent value="code" className="space-y-4">
                    <div>
                      ...
                      <Textarea id="html_code" value={formData.html_code} onChange={e => setFormData({
                    ...formData,
                    html_code: e.target.value
                  })} rows={8} className="font-mono" placeholder="Enter HTML code here..." />
                    </div>

                    <div>
                      <Label htmlFor="css_code">CSS Code</Label>
                      <Textarea id="css_code" value={formData.css_code} onChange={e => setFormData({
                    ...formData,
                    css_code: e.target.value
                  })} rows={8} className="font-mono" placeholder="Enter CSS code here..." />
                    </div>

                    <div>
                      <Label htmlFor="js_code">JavaScript Code</Label>
                      <Textarea id="js_code" value={formData.js_code} onChange={e => setFormData({
                    ...formData,
                    js_code: e.target.value
                  })} rows={8} className="font-mono" placeholder="Enter JavaScript code here..." />
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="introductions" className="space-y-4">
                    <div>
                      ...
                      <Textarea id="html_introduction" value={formData.html_introduction} onChange={e => setFormData({
                    ...formData,
                    html_introduction: e.target.value
                  })} rows={4} placeholder="Explain what this HTML code does, its features, and usage..." />
                    </div>

                    <div>
                      <Label htmlFor="css_introduction">CSS Introduction</Label>
                      <Textarea id="css_introduction" value={formData.css_introduction} onChange={e => setFormData({
                    ...formData,
                    css_introduction: e.target.value
                  })} rows={4} placeholder="Explain what this CSS code does, styling features, and effects..." />
                    </div>

                    <div>
                      <Label htmlFor="js_introduction">JavaScript Introduction</Label>
                      <Textarea id="js_introduction" value={formData.js_introduction} onChange={e => setFormData({
                    ...formData,
                    js_introduction: e.target.value
                  })} rows={4} placeholder="Explain what this JavaScript code does, functionality, and interactions..." />
                    </div>
                  </TabsContent>

                  <TabsContent value="preview" className="space-y-4">
                    {formData.html_code || formData.css_code || formData.js_code ? <CodePreview htmlCode={formData.html_code} cssCode={formData.css_code} jsCode={formData.js_code} onHtmlChange={code => setFormData({
                  ...formData,
                  html_code: code
                })} onCssChange={code => setFormData({
                  ...formData,
                  css_code: code
                })} onJsChange={code => setFormData({
                  ...formData,
                  js_code: code
                })} readonly={false} /> : <div className="text-center py-12 text-gray-500">
        ...
        <div className="grid gap-4">
          {materials.map(material => <Card key={material.id} className="overflow-hidden">
              <CardHeader className="pb-3">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                  <div className="flex flex-col sm:flex-row gap-4 flex-1 min-w-0">
                    {/* Thumbnail Display */}
                    {material.thumbnail_url && <div className="w-full sm:w-20 h-20 flex-shrink-0">
                        <img src={material.thumbnail_url} alt={material.title} className="w-full h-full object-cover rounded-lg border" onError={e => {
              ...
              <CardContent className="pt-0">
                ...
                {material.tags && material.tags.length > 0 && <div className="mt-3">
                    <div className="flex flex-wrap gap-1">
                      {material.tags.slice(0, 6).map((tag, idx) => <Badge key={idx} variant="secondary" className="text-xs">
                          {tag}
                        </Badge>)}
                      {material.tags.length > 6 && <Badge variant="secondary" className="text-xs">
        ...
        {/* Pagination Controls */}
        {totalPages > 1 && <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="text-sm text-muted-foreground text-center sm:text-left">
              Showing {startIndex + 1} to {endIndex} of {totalMaterials} materials
            </div>
            <div className="flex flex-wrap justify-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="text-xs px-3">
                Previous
              </Button>
            ...
            return <Button key={pageNum} variant={currentPage === pageNum ? "default" : "outline"} size="sm" onClick={() => setCurrentPage(pageNum)} className="text-xs px-3 min-w-[36px]">
                    {pageNum}
                  </Button>;
          })}
              
              <Button variant="outline" size="sm" onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="text-xs px-3">
                Next
              </Button>
            </div>
          </div>}
